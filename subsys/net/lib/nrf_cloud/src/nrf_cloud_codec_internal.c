					     ...
					     struct nrf_cloud_data *const output)
{
	...
	if (!IS_ENABLED(CONFIG_NRF_CLOUD_COAP)) {
		state_obj = cJSON_AddObjectToObjectCS(root_obj, NRF_CLOUD_JSON_KEY_STATE);
		reported_obj = cJSON_AddObjectToObjectCS(state_obj, NRF_CLOUD_JSON_KEY_REP);
		control_obj = cJSON_AddObjectToObjectCS(reported_obj, NRF_CLOUD_JSON_KEY_CTRL);
	} else {
		control_obj = cJSON_AddObjectToObjectCS(root_obj, NRF_CLOUD_JSON_KEY_CTRL);
	}
	...
	if (!(root_obj && control_obj && !alert_ret && !log_ret)) {
		err = -ENOMEM;
		goto end;
	}
	buffer = cJSON_PrintUnformatted(root_obj);
