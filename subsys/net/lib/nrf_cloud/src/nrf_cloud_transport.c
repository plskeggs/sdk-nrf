...
static int endp_send(const struct nct_dc_data *dc_data, struct mqtt_utf8 *endp, enum mqtt_qos qos)
{
	...
	LOG_DBG("Topic: %*s", endp->size, (const char *)endp->utf8);
	/* Populate payload. */
...
int nct_cc_send(const struct nct_cc_data *cc_data)
{
	...
	if ((cc_data->data.len != 0) && (cc_data->data.ptr != NULL)) {
		...
		LOG_DBG("mqtt_publish: id = %d opcode = %d msg: %*s",
			publish.message_id, cc_data->opcode,
			cc_data->data.len, (const char *)cc_data->data.ptr);
		LOG_DBG("    to topic: %*s", publish.message.topic.topic.size,
			(const char *)publish.message.topic.topic.utf8);
	}
