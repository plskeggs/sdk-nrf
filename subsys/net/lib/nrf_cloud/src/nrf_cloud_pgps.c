...
#define REPLACEMENT_THRESHOLD (NUM_PREDICTIONS - 2) /* (NUM_PREDICTIONS / 10) */
...
static struct nrf_cloud_pgps_prediction *storage = (struct nrf_cloud_pgps_prediction *)
...
int nrf_cloud_notify_prediction(void)
{
	...
	if ((err == -ELOADING) || (err == -EAPPROXIMATE)) {
