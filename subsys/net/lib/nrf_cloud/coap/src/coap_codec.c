			   ...
			   struct lte_lc_cells_info const *const cell_info)
{
	if (cell_info != NULL) {
		...
		if (cell_info->current_cell.id != LTE_LC_CELL_EUTRAN_ID_INVALID) {
			...
			LOG_DBG("Copied serving cell");
			cell->_cell_nmr._cell_nmr_ncells_count = cell_info->ncells_count;
			if (cell_info->ncells_count) {
				num_neighbors = MIN(ARRAY_SIZE(cell->_cell_nmr._cell_nmr_ncells),
						    cell_info->ncells_count);
				copy_ncells(cell->_cell_nmr._cell_nmr_ncells,
					    num_neighbors,
					    cell_info->neighbor_cells);
				cell->_cell_nmr._cell_nmr_ncells_count = num_neighbors;
				cell->_cell_nmr_present = true;
				LOG_DBG("Copied %zd neighbor cells", num_neighbors);
			}
			cell++;
			num_cells--;
		}
		if (cell_info->gci_cells != NULL) {
			LOG_DBG("Copying %u GCI cells", cell_info->gci_cells_count);
		...
		LOG_DBG("Copied %zd total cells",
			lte_encode->_lte_ar__cell_count + cell->_cell_nmr._cell_nmr_ncells_count);
	}
}
