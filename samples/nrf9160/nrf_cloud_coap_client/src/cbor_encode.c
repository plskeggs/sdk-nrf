...
static bool encode_repeated_location_req_lte(zcbor_state_t *state, const struct location_req_lte *input);
...
static bool encode_repeated_location_req_wifi(zcbor_state_t *state, const struct location_req_wifi *input);
		...
		zcbor_state_t *state, const struct cell *input)
{
	...
	&& (zcbor_int32_encode(state, (&(*input)._cell_mcc))))
...
static bool encode_repeated_location_req_lte(
		zcbor_state_t *state, const struct location_req_lte *input)
{
	zcbor_print("%s\r\n", __func__);
	struct zcbor_string tmp_str;

	bool tmp_result = ((((zcbor_tstr_encode(state, ((tmp_str.value = (uint8_t *)"lte", tmp_str.len = sizeof("lte") - 1, &tmp_str)))))
	&& (encode_lte_ar(state, (&(*input)._location_req_lte)))));
...
static bool encode_repeated_location_req_wifi(
		zcbor_state_t *state, const struct location_req_wifi *input)
{
	zcbor_print("%s\r\n", __func__);
	struct zcbor_string tmp_str;

	bool tmp_result = ((((zcbor_tstr_encode(state, ((tmp_str.value = (uint8_t *)"wifi", tmp_str.len = sizeof("wifi") - 1, &tmp_str)))))
	&& (encode_wifi_ob(state, (&(*input)._location_req_wifi)))));
		...
		zcbor_state_t *state, const struct location_req *input)
{
	...
	bool tmp_result = (((zcbor_map_start_encode(state, 2) && ((zcbor_present_encode(&((*input)._location_req_lte_present), (zcbor_encoder_t *)encode_repeated_location_req_lte, state, (&(*input)._location_req_lte))
	&& zcbor_present_encode(&((*input)._location_req_wifi_present), (zcbor_encoder_t *)encode_repeated_location_req_wifi, state, (&(*input)._location_req_wifi))) || (zcbor_list_map_end_force_encode(state), false)) && zcbor_map_end_encode(state, 2))));
