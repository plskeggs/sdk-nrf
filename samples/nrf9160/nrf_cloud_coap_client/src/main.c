				      ...
				      uint8_t *buf, int received)
{
	...
	LOG_INF("Got response uri:%s, code:0x%02x, type:%u %s, MID:%u, token:0x%02x%02x (len %u)",
		uri_path, code, type, coap_types[type], message_id,
		token[1], token[0], token_len);

	err = -ENOMSG;
	SYS_DLIST_FOR_EACH_CONTAINER(&con_messages, msg, node) {
		LOG_DBG("mid:%u, token:0x%02x%02x", msg->message_id, msg->token[1], msg->token[0]);
...
static int client_post_send(const char *resource, uint8_t *buf, size_t buf_len, bool cbor_fmt)
{
	int err;
	struct coap_packet request = {0};
...
void main(void)
{
	...
	uint8_t *get_payload;
#if defined(COAP_POC)
	const char *get_res = "test";
	const char *post_res = "large";
#elif defined(CELL_POS_PAYLOAD)
	const char *get_res = "cell_pos";
	const char *post_res = "temp";
#else
	const char *get_res = "ground-fix?doReply=true&mmc=260&mcc=310"
	...
	while (1) {
		if (k_uptime_get() >= next_msg_time) {
			...
			if (toggle) {
				...
				if (client_post_send(post_res, buffer, len) != 0) {
